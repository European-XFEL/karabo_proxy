...
def error_401_put(operation_name: str, device_id: str) -> str:
    return ("Lacking valid access_token with permissions to "
            f"{operation_name} of '{device_id}': "
            "please obtain a token and pass it to set_access_token.")


def error_403_put(operation_name: str, device_id: str) -> str:
    return (f"Cannot {operation_name} of "
            f"'{device_id}': check if the device is reconfigurable and"
            " the access_token has the required permissions.")


def error_422_put(operation_name: str, device_id: str) -> str:
    return (f"Cannot {operation_name} of {device_id}: "
            "the device is not reconfigurable.")
